  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], fileRejections: FileRejection[]) => {
      ...
      const invalidFiles = fileRejections.map(
        ({ file: rejectedFile, errors: rejectedErrors }) => {
          (rejectedFile as FileWithPreview).preview =
            URL.createObjectURL(rejectedFile);
          (rejectedFile as FileWithPreview).errors = rejectedErrors.map(
            (error) => ({
              code: error.code,
              message: error.message,
            })
          );
          return rejectedFile as FileWithPreview;
        }
      );
